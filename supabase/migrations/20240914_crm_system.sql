...
CREATE TABLE IF NOT EXISTS client_relationships (
    ...
    tenant_id BIGINT NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    client_id BIGINT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
    related_client_id BIGINT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
...
CREATE POLICY "Tenants can view their client activities" ON client_activities
...
CREATE POLICY "Tenants can insert their client activities" ON client_activities
    FOR INSERT WITH CHECK (tenant_id IN (
        SELECT id FROM tenants WHERE slug = current_setting('app.current_tenant', true)
    ));

DROP POLICY IF EXISTS "Tenants can update their client activities" ON client_activities;
CREATE POLICY "Tenants can update their client activities" ON client_activities
    FOR UPDATE USING (tenant_id IN (
        SELECT id FROM tenants WHERE slug = current_setting('app.current_tenant', true)
    ));

DROP POLICY IF EXISTS "Tenants can delete their client activities" ON client_activities;
CREATE POLICY "Tenants can delete their client activities" ON client_activities
...
CREATE POLICY "Tenants can manage their client communications" ON client_communications
...
CREATE POLICY "Tenants can manage their client reminders" ON client_reminders
    FOR ALL USING (tenant_id IN (
        SELECT id FROM tenants WHERE slug = current_setting('app.current_tenant', true)
    ));
...
CREATE POLICY "Tenants can manage their client notes" ON client_notes
    FOR ALL USING (tenant_id IN (
        SELECT id FROM tenants WHERE slug = current_setting('app.current_tenant', true)
    ));
...
CREATE POLICY "Tenants can manage their client relationships" ON client_relationships
    FOR ALL USING (tenant_id IN (
        SELECT id FROM tenants WHERE slug = current_setting('app.current_tenant', true)
    ));

-- Function to get client statistics
CREATE OR REPLACE FUNCTION get_client_stats(p_tenant_id BIGINT)
RETURNS TABLE (
...
-- INSERT INTO client_activities (tenant_id, client_id, type, title, description, date, outcome)
-- SELECT 
--     c.tenant_id,
--     c.id,
--     'note',
--     'Initial client setup',
--     'Client added to CRM system',
--     NOW(),
--     'completed'
-- FROM clients c
-- WHERE NOT EXISTS (
--     SELECT 1 FROM client_activities ca 
--     WHERE ca.client_id = c.id AND ca.type = 'note' AND ca.title = 'Initial client setup'
-- );
